...
function navBarSearch(e) {
	...
	function noMatch(array) {
		//error message if no match to input
		if (array.length <= 0 || array.length === 150) {
			//checks for no matchs or ALL matchs (meaning no input provided but search ran)
	...
	function addToDropdown(pokemon) {
		if (filteredPokemon.length > 0 && filteredPokemon.length != 150) {
			//checks for matches and makes sure it's not all items in array
